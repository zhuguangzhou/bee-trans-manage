...
public class DynamicDataSourceRegister implements ImportBeanDefinitionRegistrar, EnvironmentAware {
    ...
    public void registerBeanDefinitions(AnnotationMetadata annotationMetadata, BeanDefinitionRegistry beanDefinitionRegistry) {
        ...
        defauleDataSourceProperties = binder.bind("druid.datasource.master", Map.class).get();
        // 获取数据源类型
        String typeStr = evn.getProperty(DATASOURCE_TYPE_DEFAULT);
        // 获取数据源类型
        ...
        // 获取其他数据源配置
        List<Map> configs = binder.bind("druid.datasource.cluster", Bindable.listOf(Map.class)).get();
        // 遍历从数据源
        for (int i = 0; i < configs.size(); i++) {
            ...
            String key = config.get("names").toString();
